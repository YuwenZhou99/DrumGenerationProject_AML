...
class DrumLogisticRegression(nn.Module):
    ...
    def forward(self, x, hidden=None):
        """
        x: (B, T, K)
        hidden: ignored, for API compatibility with RNNs
        output: (logits, None) to match the RNN's output signature
        """
        out = self.linear(x)
        return out, None
...
def load_model(path: str, model: nn.Module, map_location="cpu"):
    # Add weights_only=False to safely load checkpoints with metadata
