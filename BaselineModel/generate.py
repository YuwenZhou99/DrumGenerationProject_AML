...
def main():
    ...
    model_path = "models/drum_log.pt"
    out_folder = "generated_samples_log"
    ...
    if "model_state" in ck and "linear.weight" in ck["model_state"]:
        ...
        if master_cols is None:
            raise RuntimeError("Could not determine the number of features (k) from the model checkpoint.")
